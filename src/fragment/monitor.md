...
document.addEventListener('click', (e) => {
    ...
    const identifier = target.id || target.className || target.getAttribute('data-tracker');
    if (identifier) {
...
window.fetch = function(...args) {
    ...
    return originalFetch.apply(this, args)
        .then(res => {
            ...
            console.log(`请求 ${args[0]} 成功，耗时 ${duration}ms, 状态码 ${res.status}`);
            // 上报成功数据
            return res;
        })
        .catch(err => {
